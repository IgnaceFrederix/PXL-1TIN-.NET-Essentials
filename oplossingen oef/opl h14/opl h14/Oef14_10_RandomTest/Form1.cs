...
namespace Oef14_10_RandomTest
{
    public partial class Form1 : Form
    {
        ...
        private void Button1_Click(System.Object sender, System.EventArgs e)
        {
            ...
            // Dit verandert tov oefening 9: 100x herhalen
            for (int i = 1; i <= 100; i++)
            {
                rij = MaakRij(aantal);
                BerekenFrequenties(rij, freq);
            }
            TekenStaafDiagram(PictureBox1, freq);
        }
        ...
        private void TekenStaafDiagram(PictureBox pbox, int[] rij)
        {
            ...
            // Merk op: tov vorig oefening werken we nu met een Double
            // omdat we hier delen door grote getallen geeft een afronding naar int hier 0
            double u_height = pbox.Height / (Max(rij) * 1.2);
            ...
            for (int i = 0; i < rij.Length; i++)
            {
                area.FillRectangle(brush, (2 * i + 1) * u_width,
                                   Convert.ToInt32(pbox.Height - rij[i] * u_height),
                                   u_width,
                                   Convert.ToInt32(rij[i] * u_height));
            }
        }
