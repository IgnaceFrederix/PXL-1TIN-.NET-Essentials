    ...
    public partial class Form1 : Form
    {
        ...
        private void updateButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string sql = "Select Company from Companies where Company = '" + companyTextBox.Text + "'";
                OleDbDataAdapter companyAdapter = new OleDbDataAdapter(sql, connection);
                companyAdapter.Fill(table);
                if (table.Rows.Count == 0)
                {

                    if (MessageBox.Show(companyTextBox.Text + " bestaat niet, toevoegen?", "Toevoegen bedrijf", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        companyAdapter.InsertCommand = new OleDbCommand("insert into Companies(Company,Location) values('" + companyTextBox.Text + "','None')");
                        companyAdapter.InsertCommand.Connection = connection;
                        companyAdapter.InsertCommand.ExecuteNonQuery();
                    }
                }
